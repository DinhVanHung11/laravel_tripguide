...
$('#form-booking .info-extra-futures input').each(function(){
    $(this).on('click', function(){
        ...
        formElements.totalInput.val(totalPrice); //Set New Total Price
        formElements.formBooking.find('.total-price-text').text(`$${formElements.totalInput.val()}`);
    });
});
...
function displayPrice(roomPrice, totalOldPrice){
    ...
    formElements.formBooking.find('.percent-sale').text(`${discount ?? '0' }%`);
    formElements.formBooking.find('.info-price .price-for-night-label').text(`${dayBooking ? dayBooking : '1'} Nights`);
    formElements.formBooking.find('.info-price .price-for-night-number').text('$'+ totalOldPrice);
    formElements.formBooking.find('.price-discount-percent').text(`Discount ${discount ?? '0' }%`);
    formElements.formBooking.find('.price-discount-number').text(`$${totalOldPrice - formElements.totalInput.val()}`);
    formElements.formBooking.find('.total-price-text').text(`$${formElements.totalInput.val()}`);

    if(discount != 0){
        formElements.formBooking.find('.percent-sale').text(`${discount}% OFF`);
